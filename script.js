...
const PROJECTS = [
  ...
  // ── ПРОЕКТ 8 ── ВТБ приложение ──────────────────────────────
  {
    ...
    context: 'ВТБ приложение — мобильный банк для повседневных финансовых операций: переводы, платежи, вклады и кредитные продукты. Основная задача — сделать частые ежедневные сценарии максимально быстрыми и понятными, сохранив доверие к банковским операциям.',

    problem: 'Пользователи тратили много времени на выполнение простых, но частых операций — переводы и оплату услуг. Основные проблемы:\n— перегруженная главная страница с большим количеством второстепенных элементов\n— долгий путь к часто используемым операциям\n— слабая персонализация под привычные сценарии пользователя\n\nВ результате: снижалась скорость выполнения ключевых действий и росло раздражение от интерфейса.',

    solution: 'Цель: сократить путь пользователя к часто используемым операциям и повысить скорость выполнения ежедневных сценариев без потери доверия к банковским процессам.\n\nРешение:\n\n1. Персонализированная главная страница — вынесены наиболее частые операции пользователя в верхний блок экрана, структура адаптируется под индивидуальное поведение.\n\n2. Сокращение шагов в ключевых сценариях — упрощены переводы и платежи: меньше промежуточных экранов, предзаполнение данных на основе истории операций.\n\n3. Ясная визуальная иерархия — усилен контраст между основными и второстепенными действиями, снижена когнитивная нагрузка при первом взгляде на экран.\n\n4. Прозрачность статусов операций — пользователь видит статус перевода или платежа в реальном времени, без необходимости уточнять его вручную.',

    keyScenarios: 'Ключевые сценарии: быстрый перевод по номеру телефона, оплата услуг ЖКХ и мобильной связи, открытие вклада с калькулятором доходности, просмотр истории операций с фильтрацией и повторение шаблонных платежей.',

    userFlow: 'Вход по биометрии → персонализированная главная страница → выбор частой операции одним касанием → подтверждение → мгновенный статус выполнения.',

    designSystem: 'Строгий корпоративный стиль банка с акцентом на читаемость и доверие: крупная типографика для сумм, фирменный синий как основной акцент, минимум декоративных элементов.',

    prototype: 'Прототип проведён через сценарий перевода по номеру телефона и оплаты услуги — от главной страницы до подтверждения операции.',

    results: 'После изменений ключевые ежедневные операции стали выполняться быстрее за счёт сокращения количества шагов и персонализации главной страницы. Повысилась предсказуемость интерфейса и снизилась нагрузка на поддержку по вопросам статуса операций.',

    tags: 'UX Research, Product Design, Fintech, Mobile Banking, Figma',
  },

...
const PROJECTS = [
  ...
  // ── ПРОЕКТ 8 ── ВТБ приложение ──────────────────────────────
  {
    ...
    context: 'Блок «Инвестиции» в приложении ВТБ — сервис «Интеллект» с автоматическим подбором инвестиционной стратегии на основе анкеты риск-профилирования. Пользователь проходит анкету из ~8 вопросов, получает персональную стратегию и переходит к подписанию документов на открытие брокерского счёта.',

    problem: 'При анализе флоу анкеты и подбора стратегии выявлены три взаимосвязанные проблемы:\n\n1. Предзаполненные ответы в ключевой анкете — на нескольких экранах по умолчанию уже выбран самый агрессивный или самый узкий вариант (высокий риск, наличие крупного кредита, минимальный доход). Анкета определяет юридически значимый риск-профиль, а пользователь, листающий форму быстро, рискует получить продукт, не соответствующий его реальной ситуации.\n\n2. Несогласованные цифры доходности по воронке — на входе заявлено «до 51%», на карточке готовой стратегии — «до 30,21%», а график падения доходности под этим числом не сопровождается пояснением периода и природы просадки.\n\n3. Разрыв между длинной анкетой и подписанием документов — нет экрана-подтверждения того, что было указано; исправить ошибку можно только пролистав анкету заново.',

    solution: 'Цель: снизить риск несоответствия подобранной стратегии реальному профилю пользователя и повысить доверие к цифрам доходности перед подписанием документов.\n\nРешение — фича «Осознанный выбор»:\n\n1. Убраны все предзаполненные ответы в анкете — кнопка «Продолжить» неактивна, пока не сделан явный выбор.\n\n2. Добавлен экран-саммари перед подписанием документов — короткий список указанных ответов (цель, риск, срок, доход) с возможностью точечно поправить любой пункт в один тап, без прохождения анкеты заново.\n\n3. Синхронизированы цифры доходности по всей воронке — входной тизер приведён в соответствие с диапазоном подобранной стратегии, график на карточке стратегии дополнен пояснением периода и природы показателя.',

    keyScenarios: 'Ключевые сценарии: прохождение анкеты риск-профилирования, подбор персональной стратегии сервисом «Интеллект», проверка и правка ответов на экране-саммари, подписание документов на открытие брокерского счёта.',

    userFlow: 'Анкета риск-профилирования (без предзаполненных ответов) → карточка персональной стратегии с согласованными цифрами доходности → экран-саммари с возможностью правки → подписание документов.',

    designSystem: 'Строгий корпоративный стиль банка с акцентом на читаемость и доверие: крупная типографика для сумм и процентов, фирменный синий как основной акцент, минимум декоративных элементов.',

    prototype: 'Прототип проведён через полный сценарий: анкета → результат стратегии → экран-саммари с правкой ответа → подписание документов.',

    results: 'Ожидаемый эффект фичи:\n— рост конверсии из «стратегия подобрана» в «документы подписаны» за счёт удобных правок и согласованных цифр доходности;\n— снижение доли жалоб «стратегия не соответствует ожиданиям» в первый месяц использования;\n— снижение оттока клиентов «Интеллекта» в первые 30 дней, особенно среди изначально консервативных профилей.',

    tags: 'UX Research, Product Design, Fintech, Investments, Figma',
  },

...
const TRANSLATIONS_EN = {
  ...
  8: {
    context: 'Transfers and payments are one of the most frequent scenarios in a mobile bank: users send money to close ones, pay for services, and repeat the same operations every day. Even so, the current flow makes them go through the full path each time — find the right section, choose the operation type, fill in the data, and confirm the transfer.\n\nGoal: reduce the time it takes to complete a transfer and cut the number of actions needed to finish an operation.\n\nMain research question: how can a user complete a familiar transfer faster, without having to search for the right scenario every single time?',

    problem: 'In the current scenario, the user has to find the needed action on their own:\nHome → Payments & Transfers → Choose operation → Fill in the data → Confirmation\n\nThe path takes several steps and creates extra drop-off points. A UX audit showed: users pass through several screens before starting an operation, there\'s no quick access to frequent recipients, a repeat transfer requires repeating most of the steps, and the key action isn\'t prioritized on the home screen — it\'s dominated by marketing blocks (promotions, bonuses, offers), even though completing an operation is the user\'s most frequent task. The app stores transfer history but doesn\'t use it to speed up the scenario or offer a hint in advance.',

    solution: 'Review mining across VTB app reviews surfaced three recurring patterns: it\'s hard to quickly find the right section (search), repeat operations feel too long (speed), and frequent actions require extra navigation steps. A comparison with Tinkoff and Sber showed that recent recipients, quick actions, and repeat transfers are more visible for competitors and take fewer steps.\n\nKey insight: users don\'t need a new way to transfer money — they need the fastest path to an action they already know. Most transfers are recurring scenarios: the same recipient, the same amounts, regular payments. So the app can use transaction history to suggest the action in advance.\n\nHypothesis: if frequently used operations are surfaced on the home screen and the app uses the user\'s history, they\'ll be able to complete a transfer faster, increasing the number of successfully completed operations.\n\nFrom the list of hypotheses, the first iteration prioritized solutions with the highest impact and lowest effort: quick actions on the home screen, repeating the last transfer, fast recipient search, and predictive payments.\n\n1. Quick transfers on the home screen ★★★★★\nAdded quick actions next to the balance: transfer, pay, QR, top up. The path shortens from "Home → Payments → Transfer → Recipient" to "Home → Recipient → Confirmation."\n\n2. A "Frequent operations" block ★★★★★\nInstead of leading with marketing offers, the home screen now surfaces the user\'s real tasks first. New structure: Balance → Quick actions → Frequent operations → Personal offers. For example: "Mom — last transfer: ₽500 yesterday" with a "Repeat" button.\n\n3. One-tap payment ★★★★★\nFor recurring operations, removed the need to re-enter data: the user immediately sees the recipient, amount, and date of the last transfer and confirms with a single "Repeat transfer" button.\n\n4. Smart action search\nSearch became a way to complete a task, not just find a section. Searching "Ivan" instantly shows a card — "Ivan P. — last transfer ₽1,500" with a "Transfer" button; searching "MTS" shows a "Pay MTS" card with a "Continue" button.\n\n5. Personal hints\nUsing transaction history for prompts like "You usually pay for internet on the 25th" with a "Pay now" button — the app helps users complete a habitual task before they even start searching for it.',

    userFlow: 'Before:\nHome → Payments & Transfers → Choose operation → Fill in the data → Confirmation\n\nAfter:\nHome → Last recipient → Confirmation\n\nReduction: 5–6 steps → 2–3 steps.',

    designSystem: 'To support fast-operation scenarios, I used: cards for recent recipients, compact action buttons, a bottom sheet for choosing the operation type, success/error states, and personal hints based on transaction history.',

    results: 'Validating the solution requires UX testing and an A/B experiment.\n\nMetrics:\n— Time To Complete Payment — time to complete a transfer\n— Task Success Rate — share of successfully completed operations\n— Drop-off Rate — share of users who left the scenario\n— Number of Steps — number of actions needed to finish an operation\n\nExpected effect: shorter transfer completion time, fewer drop-offs in the scenario, a higher share of successful operations, and a more convenient day-to-day app experience.\n\nA good mobile bank doesn\'t just store transaction history — it helps users complete familiar tasks faster.',
  },
...
const PROJECTS = [
  ...
  // ── ПРОЕКТ 8 ── ВТБ Оплата ──────────────────────────────
  {
    ...
    context: 'Переводы и платежи — один из самых частых сценариев в мобильном банке: пользователи ежедневно переводят деньги близким, оплачивают услуги и повторяют одни и те же операции. При этом текущий сценарий каждый раз требует пройти полный путь: найти нужный раздел, выбрать тип операции, заполнить данные и подтвердить перевод.\n\nЦель: сократить время выполнения перевода и уменьшить количество действий, необходимых для завершения операции.\n\nОсновной вопрос исследования: как помочь пользователю выполнить привычный перевод быстрее, не заставляя его каждый раз искать нужный сценарий заново?',

    problem: 'В текущем сценарии пользователь должен самостоятельно найти нужное действие:\nГлавная → Платежи и переводы → Выбор операции → Заполнение данных → Подтверждение\n\nПуть занимает несколько шагов и создаёт дополнительные точки отказа. UX-аудит показал: пользователь проходит несколько экранов перед началом операции, нет быстрого доступа к частым получателям, повторный перевод требует повторения большинства шагов, а ключевое действие не является приоритетным на главном экране — там преобладают маркетинговые блоки (акции, бонусы, предложения), хотя самая частая задача пользователя — совершить операцию. Приложение хранит историю переводов, но не использует её, чтобы ускорить сценарий и предложить подсказку заранее.',

    researchImage: 'assets/Исследование_VTB.jpg',

    solution: 'Review mining отзывов о приложении ВТБ показал три повторяющихся паттерна: пользователям сложно быстро найти нужный раздел (поиск операций), повторные операции воспринимаются как слишком длинные (скорость выполнения), частые действия требуют лишних переходов (навигация). Сравнение с Т-Банком и Сбером показало, что у конкурентов последние получатели, быстрые действия и повтор операции реализованы заметнее и требуют меньше шагов.\n\nГлавный инсайт: пользователю не нужен новый способ перевода — ему нужен самый быстрый путь к уже знакомому действию. Большая часть переводов — это повторяющиеся сценарии: один и тот же получатель, одинаковые суммы, регулярные платежи. Значит, приложение может использовать историю операций, чтобы предлагать действие заранее.\n\nГипотеза: если вынести часто используемые операции на главный экран и использовать историю действий пользователя, он сможет завершать перевод быстрее, что повысит количество успешно завершённых операций.\n\nИз списка гипотез в первую итерацию выбрала решения с максимальным влиянием и минимальной сложностью реализации: быстрые действия на главном экране, повтор последнего перевода, быстрый поиск получателя, предиктивные платежи.\n\n1. Быстрые переводы на главном экране ★★★★★\nДобавила быстрые действия рядом с балансом: перевести, оплатить, QR, пополнить. Путь сокращается с «Главная → Платежи → Перевод → Получатель» до «Главная → Получатель → Подтверждение».\n\n2. Блок «Частые операции» ★★★★★\nВместо того чтобы сначала показывать маркетинговые предложения, главный экран сразу предлагает пользователю его реальные задачи. Новая структура: Баланс → Быстрые действия → Частые операции → Персональные предложения. Например: «Мама — последний перевод: 500 ₽ вчера» и кнопка «Повторить».\n\n3. One Tap Payment ★★★★★\nДля повторяющихся операций убрала повторный ввод данных: пользователь сразу видит получателя, сумму и дату последнего перевода и подтверждает операцию одной кнопкой «Повторить перевод».\n\n4. Умный поиск действий\nПоиск стал не только поиском разделов, а инструментом выполнения задачи. Запрос «Иван» сразу показывает карточку «Иван П. — последний перевод 1500 ₽» с кнопкой «Перевести», запрос «МТС» — карточку «Оплатить МТС» с кнопкой «Продолжить».\n\n5. Персональные подсказки\nИспользование истории операций для подсказок вида «Обычно вы оплачиваете интернет 25 числа» с кнопкой «Оплатить сейчас» — приложение помогает выполнить привычную задачу до того, как пользователь начнёт искать её сам.',

    userFlow: 'Было:\nГлавная → Платежи и переводы → Выбор операции → Заполнение данных → Подтверждение\n\nСтало:\nГлавная → Последний получатель → Подтверждение\n\nСокращение: 5–6 шагов → 2–3 шага.',

    designSystem: 'Для поддержки сценариев быстрых операций использовала: карточки последних получателей, компактные action-кнопки, bottom sheet для выбора типа операции, состояния успешного/ошибочного действия и персональные подсказки на основе истории.',

    uikitImage: 'assets/Design System Sections VTB.png',

    results: 'Для оценки эффективности решения необходимо провести UX-тестирование и A/B эксперимент.\n\nМетрики:\n— Time To Complete Payment — время выполнения перевода\n— Task Success Rate — процент успешно завершённых операций\n— Drop-off Rate — доля пользователей, покинувших сценарий\n— Number of Steps — количество действий до завершения операции\n\nОжидаемый эффект: сокращение времени выполнения переводов, снижение количества отказов в сценарии, рост доли успешных операций и повышение удобства ежедневного использования приложения.\n\nХороший мобильный банк не просто хранит историю операций — он помогает пользователю быстрее выполнять привычные задачи.',
